// ============================================================
// CONTACT.JS — Contact form with validation + Google Sheets
// Copied as-is — uses native form validation, no Bootstrap JS dependency
// ============================================================
...
function customValidation() {
  ...
  const phoneCol = phoneInput.closest("div");
  const phoneInvalidFeedback = phoneCol?.querySelector(".invalid-feedback");
  ...
  if (!form.checkValidity()) {
    ...
    invalidFields.forEach((field) => {
      ...
      if (field.type === "radio" || field.type === "checkbox") {
        elementToShake = field.closest("label");
      } else {
        elementToShake = field.closest(".mb-4") || field.closest("div");
      }
...
function handleRealtimeValidation(inputElement) {
  ...
  if (inputElement.type === "radio" || inputElement.type === "checkbox") {
    elementToShakeParent = inputElement.closest("label");
  } else {
    elementToShakeParent =
      inputElement.closest(".mb-4") || inputElement.closest("div");
  }
